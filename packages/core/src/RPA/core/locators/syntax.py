...
# Human-friendly names for tokens
NAMES = {
    Token.THEN: "then",
    Token.AND: "and",
    Token.OR: "or",
...
class Expression(NamedTuple):
    ...
    def __repr__(self):
        return f"Expression({self.lhs} {NAMES[self.op]} {self.rhs})"
...
class SyntaxParser:
    ...
    def _expression(self) -> ValueType:
        ...
        expr = self._value()
        while not self.tokens.is_empty and self.tokens.peek.token in (
            Token.AND,
            Token.OR,
        ):
            op = self._consume()
    ...
    def _expect(self, token: Token):
        ...
        if not self._accept(token):
            expected = NAMES[token]
            actual = NAMES[self.tokens.peek.token]
            raise InvalidSyntax(f"Expected '{expected}', was '{actual}'")
